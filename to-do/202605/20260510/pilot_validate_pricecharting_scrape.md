...
- [x] Add `python/test/scrape/test_pilot_pricecharting_scrape.py` (constants + live limited `scrape_pricecharting_data`, no mocks).
- [x] Extend `scrape_pricecharting_data` with `max_sets`, `sleep_seconds`, `quiet` for pilots outside Streamlit.
- [x] Run `poetry run pytest test/scrape/test_pilot_pricecharting_scrape.py -v` from `python/`.
