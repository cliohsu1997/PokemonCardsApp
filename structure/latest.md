...
| `python/test/` | **All** `pytest` modules (`poetry run pytest test\ -v`); **`conftest.py`** adds **`python/code/`** to `sys.path`. |
| `python/test/scrape/` | Scrape-only tests (e.g. **`test_pilot_pricecharting_scrape.py`**). |
